...
このSkillは、VS Code の **Agent hooks** を設計し、ワークスペースへ追加する。Hooksはエージェントセッションのライフサイクルポイントでシェルコマンドを確定的に実行し、セキュリティ強制・コード品質自動化・監査ログなどを実現する。Hooks は Preview 機能のため仕様変更の可能性がある。
...
- Claude Code の matcher 付き設定は読み込まれるが、VS Code では matcher 値を無視して全ツールに適用される
