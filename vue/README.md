# Vue + GraphQL + Prisma テンプレート

Vue 3 + TypeScript + Docker + GraphQL (urql) + Prisma のサンプル環境テンプレートです。このフォルダをコピーして、すぐに新しいプロジェクトを始められます。

## 🎯 概要

このテンプレートは、練習用・学習用の Vue アプリケーション環境を提供します。
...
- **VueUse 11.3** - Composition API ユーティリティ
...
**注意**: Vue は純粋な CSR フレームワークのため、完全な SSR はサポートしていません。
...
```bash
# PostgreSQLコンテナに接続
docker-compose exec postgres psql -U postgres -d vue_db
...
- `src/App.test.ts` - アプリケーションのテスト
- `src/views/HomePage.test.ts` - ホームページのテスト
- `src/stores/counter.test.ts` - Pinia store のテスト
...
DATABASE_URL=postgresql://postgres:postgres@postgres:5432/vue_db?schema=public
```
...
A: Vue は純粋な CSR フレームワークのため、完全な SSR はサポートしていません。SSR が必要な場合は、**Nuxt.js**の使用を推奨します。このテンプレートでは、バックエンド API 経由でのデータ取得を想定しています。
...
A: Express 等のバックエンド API サーバーを実装し、そこから Prisma を使用してください。フロントエンド（Vue）からは Axios 等で API エンドポイントを呼び出します。
...
- [VueUse ドキュメント](https://vueuse.org/)

### テスト

- [Vitest 公式ドキュメント](https://vitest.dev/)
- [Vue Test Utils 公式ドキュメント](https://test-utils.vuejs.org/)
...
- **react/** - React + urql (CSR) + Prisma (API 経由) テンプレート
