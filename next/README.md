...
- **React Testing Library 16.1.0** - Reactコンポーネントテスト
- **jsdom 25.0.1** - DOM環境シミュレーション
...
│   │   ├── store.ts      # Store設定
│   │   └── slices/       # Redux slices
│   │       └── counterSlice.test.ts  # counterスライスのテスト
│   ├── test/             # テスト設定
│   │   ├── setup.ts      # テストセットアップ
...
- `src/app/graphql/page.test.tsx` - GraphQLページのテスト
- `src/store/slices/counterSlice.test.ts` - Redux storeのテスト
