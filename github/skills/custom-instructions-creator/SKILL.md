...
配置場所の既定: ワークスペースは `.github/instructions/`、ユーザープロファイルは VS Code の現在プロファイル配下の prompts フォルダ。追加の場所は `chat.instructionsFilesLocations` で拡張できる。
...
`.claude/rules` の instructions 互換ファイルは、`applyTo` ではなく `paths`（配列）でパターンを指定する。`paths` を省略した場合は `**`（全ファイル）が既定。
